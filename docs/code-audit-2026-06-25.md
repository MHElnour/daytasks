...
| 2026-06-25 | BAD-10 | _this commit_ | Clickable tag/project chips get `role="button"` + `tabindex=0` + Enter/Space activation via a shared `makeActivatable` helper, so keyboard / screen-reader users can trigger them. TDD: chip keyboard case in `tests/obsidian/widgetRenderer.test.ts`. |
