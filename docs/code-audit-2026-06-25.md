...
| 2026-06-25 | BAD-6 | _this commit_ | `formatDailyTaskLine`/`upsertDailyTaskLine` now take a `completed` flag (caller supplies via `isCompletedStatus`) instead of matching the literal `"done"`. TDD: formatter + document tests updated. Module still unwired (roadmap). |
| 2026-06-25 | DRY-1, DRY-2, DRY-5 | _this commit_ | Shared `util/parseLabelList` replaces `parseList`/`parseTags`; `DayTaskService.applyCompletion` de-duplicates the completedAt logic; `obsidian/projectPicker.addMarkdownPathPicker` removes the duplicated picker wiring. TDD for `parseLabelList`; service + picker covered by existing tests / build. |
