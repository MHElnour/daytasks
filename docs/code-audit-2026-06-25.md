...
| 2026-06-25 | OPT-5 | _this commit_ | `MarkdownPathSuggestModal.getItems()` caches the markdown-path list for the modal's lifetime instead of rescanning + re-sorting the whole vault on every keystroke. Verified by build (Obsidian UI glue). |
