...
| 2026-06-25 | BAD-5 | _this commit_ | New `obsidian/globalSearch.openGlobalSearch` contains the private `internalPlugins` cast in one feature-detected, typed adapter; `searchTag` falls back to a Notice when global search is unavailable. TDD: 4 cases in `tests/obsidian/globalSearch.test.ts`. |
